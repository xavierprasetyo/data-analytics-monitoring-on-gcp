# Monitoring & Alerting Cost Analysis — Airflow 2.11.1

> **Environment:** `composer-3-airflow-2.11.1-build.5`
>
...
| # | Policy | Condition Type | Severity |
|---|---|---|---|
| 1 | Failed DAG Runs | `condition_threshold` | CRITICAL |
| 2 | Failed Task Instances | `condition_threshold` | CRITICAL |
| 3 | Scheduler Heartbeat Missing | `condition_absent` | CRITICAL |
| 4 | Worker Pod Evictions | `condition_threshold` | WARNING |
| 5 | Database Health Degraded | `condition_threshold` | WARNING |

#### Policies from `logging.tf` (2 policies, 1 condition each)

| # | Policy | Condition Type | Severity |
|---|---|---|---|
| 6 | Error Logs Detected | `condition_threshold` (on log-based metric) | CRITICAL |
| 7 | DAG Parse Errors | `condition_threshold` (on log-based metric) | ERROR |
...
- This version uses the `severity` field on alert policies (e.g., `CRITICAL`, `WARNING`, `ERROR`), which is supported in the Terraform Google provider. This does **not** affect pricing.
- SLA-based monitoring is **available** in Airflow 2.x but not configured in this lab. Adding SLA alerts would add $1.50/month per condition.
