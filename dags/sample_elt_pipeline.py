# DAG version: 2026-05-22-v1 (Airflow 2.11.1 on Composer 3)
"""
...
MONITORING (Airflow 2.11.1):
  - Every task has on_failure_callback → sends structured error logs
  - The DAG has on_failure_callback → sends DAG-level failure summary
  - SLA feature is available in Airflow 2.x (set `sla` on tasks if needed)
  - execution_date is deprecated → use logical_date (available since 2.2)

COMPATIBILITY:
  - Target: composer-3-airflow-2.11.1-build.5
  - `schedule` parameter used (supported since Airflow 2.4)
  - `schedule_interval` is deprecated but still works
"""
...
with DAG(
    ...
    schedule="0 6 * * *",  # Daily at 6 AM UTC (schedule param works since Airflow 2.4)
    catchup=False,
    max_active_runs=1,
    ...
    def _notify_success(**context):
        ...
        dag_obj = context.get('dag', context.get('dag_run', None))
        dag_id = dag_obj.dag_id if hasattr(dag_obj, 'dag_id') else str(dag_obj)
        run_id = context.get('run_id', 'N/A')
        # Use logical_date (preferred) with execution_date as fallback
        logical_date = context.get('logical_date', context.get('execution_date', 'N/A'))
        print("=" * 50)
